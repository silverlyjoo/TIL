...
def robot(sy, sx, sdir, ey, ex, edir):
    ...
    while que:
        ...
        for idx1 in range(1, 4):
            ...
            if not iswall(ny, nx) and [ny, nx, ddir] not in visit:
                que.append([ny, nx, ddir, turn+1])
                visit.append([ny, nx, ddir])
        
        if [y, x, turns[ddir][0]] not in visit:
            que.append([y, x, turns[ddir][0], turn+1])
            visit.append([y, x, turns[ddir][0]])
        if [y, x, turns[ddir][1]] not in visit:
            que.append([y, x, turns[ddir][1], turn+1])
            visit.append([y, x, turns[ddir][1]])
